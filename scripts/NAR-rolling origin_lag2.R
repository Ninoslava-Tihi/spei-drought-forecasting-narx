...
for(i in initial_train:(n-1)){
  ...
  fc_lag2 <- forecast(model_lag2, h = 1)

  
  # Store prediction and actual value
  nar_predictions_lag2 <- c(nar_predictions_lag2, as.numeric(fc_lag2$mean))
...
# ==========================================
rmse_val <- rmse(actuals, nar_predictions_lag2)
mae_val  <- mae(actuals, nar_predictions_lag2)
r2_val   <- cor(actuals, nar_predictions_lag2)^2
...
plot(actuals,
     ...
     lwd = 3,
     col = "black",
     ylim = range(c(actuals, nar_predictions_lag2)),
     main = "NAR Rolling Origin: Real vs Predicted SPEI",
     xlab = "Forecast step",
     ylab = "SPEI")

lines(nar_predictions_lag2,
      col = "red",
      lwd = 2,
      lty = 2)
