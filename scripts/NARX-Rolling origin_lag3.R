...
for(i in initial_train:(n-1)){
  ...
  fc_lag3 <- forecast(model_lag3, xreg = xreg_test, h = 1)
  
  # Store prediction and real value
  narx_predictions_lag3 <- c(narx_predictions_lag3, as.numeric(fc_lag3$mean))
...
rmse_val <- rmse(actuals, narx_predictions_lag3)
mae_val  <- mae(actuals, narx_predictions_lag3)
r2_val   <- cor(actuals, narx_predictions_lag3)^2
...
plot(actuals,
     ...
     lwd = 3,
     col = "black",
     ylim = range(c(actuals, narx_predictions_lag3)),
     main = "NARX Rolling Origin: Real vs Predicted SPEI",
     xlab = "Forecast step",
     ylab = "SPEI")

lines(narx_predictions_lag3,
      col = "blue",
      lwd = 2,
      lty = 2)
